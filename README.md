...
### AWS CLI no reconocido en VS Code

Se solucionó revisando la instalación de AWS CLI y el PATH de Windows.

### Error al subir a GitHub por archivo demasiado grande

GitHub rechazó el push porque se intentó subir la carpeta `.terraform/`, que contiene proveedores pesados.
...
### Error en DynamoDB por tipo de dato
...
```json
"Timestamp": {
  "S": "2026-05-28T20:30:00"
}
```

### Error por Log Group existente en CloudWatch

Lambda ya había creado automáticamente el Log Group:

```text
/aws/lambda/rapiro-lsa-inference
```

Se solucionó importándolo al estado de Terraform:

```powershell
terraform import aws_cloudwatch_log_group.lambda_inference_logs "/aws/lambda/rapiro-lsa-inference"
```
...
* `terraform output`.
* Bucket S3 con carpetas `tests/` y `audio/`.
* Tabla DynamoDB con registros.
* Lambda `rapiro-lsa-inference`.
* Variables de entorno de Lambda.
* Rol IAM asociado a Lambda.
* Política IAM con permisos.
* Audio MP3 generado en S3.
* Logs de CloudWatch.
* IoT Core Thing.
* IoT Topic Rule.
* MQTT Test Client publicando al topic `rapiro/lsa/keypoints`.

---

## Próximos pasos del proyecto

Con la demo cloud ya funcional, el próximo objetivo es integrar la nube con la aplicación Python, MediaPipe y RAPIRO.

Orden recomendado:

1. Crear un script Python que publique mensajes simulados al topic MQTT.
2. Probar `Python local → IoT Core → Lambda`.
3. Integrar MediaPipe Hands.
4. Extraer los 21 puntos clave de la mano.
5. Generar el vector de 63 valores.
6. Conectar el modelo MLP del alfabeto LSA.
7. Enviar predicciones reales a AWS IoT Core.
8. Reproducir el audio generado por Polly.
9. Medir latencia y errores con CloudWatch.
10. Preparar la demo final con notebook o RAPIRO.

---

## Comandos de limpieza

Para eliminar la infraestructura creada por Terraform:
...
Precaución: este comando elimina los recursos administrados por Terraform. Antes de ejecutarlo, respaldar archivos importantes de S3 y registros relevantes de DynamoDB.
